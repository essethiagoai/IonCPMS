      ...
      <div className="map-wrapper">
        {isApiKeyValid ? (
          ...
          <div style={{ 
            ...
            <p>Para visualizar o mapa, configure uma API key válida do Google Maps.</p>
            <p><strong>Locais encontrados:</strong> {locations.length}</p>
            <p><strong>Estações encontradas:</strong> {allMarkers.length}</p>
          </div>
        )}
      </div>
    </CanAccess>
...
const MapWithClustering: React.FC<{
  ...
  return (
    <GoogleMap
      {...(config.googleMapsOverviewMapId && { mapId: config.googleMapsOverviewMapId })}
      defaultCenter={defaultCenter}
